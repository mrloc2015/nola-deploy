    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            ...
            if (!$input->getOption('skip-upgrade') && $changeAnalyzer->needsSetupUpgrade($changes)) {
                $logger->info('  ▸ setup:upgrade');
            }
            if (!$input->getOption('skip-di') && $changeAnalyzer->needsDiCompile($changes)) {
                $logger->info('  ▸ setup:di:compile');
            } else {
                $logger->info('  ✓ di:compile skipped (no PHP/DI changes)');
            }
            if (!$input->getOption('skip-static') && $changeAnalyzer->needsStaticDeploy($changes)) {
                $themeCodes = array_map(fn($t) => $t->code, $themes);
                $changedThemeCodes = $changeAnalyzer->getChangedThemes($changes, $themeCodes);
                if (!empty($changedThemeCodes) && !$force) {
                    $logger->info('  ▸ static-content:deploy → ' . implode(', ', $changedThemeCodes));
                } else {
                    $logger->info('  ▸ static-content:deploy → all themes');
                }
            } else {
                $logger->info('  ✓ static-content:deploy skipped (no theme changes)');
            }
            ...
            if (!$input->getOption('skip-static') && $changeAnalyzer->needsStaticDeploy($changes)) {
                ...
                if (!empty($changedThemeCodes) && !$force) {
                    ...
                    if (!empty($deployThemes)) {
                        $logger->info('Deploying only changed themes: ' . implode(', ', $changedThemeCodes));
                    }
                }
                ...
                $logger->info('Static content deploy skipped (no changes detected)');
            }
